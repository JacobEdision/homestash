...
export default function App() {
  ...
  const sp={T,tags,rooms,storages,containers,items,canEdit,canAdmin,isSuperAdmin,filterConsumable,setFilterConsumable,filteredItems,setModal,deleteItem:confirmDeleteItem,rawDeleteItem:deleteItem,returnItem,markOos,nav,guestCanSeeStorage,guestCanSeeContainer};
...
// ─── ITEM CARD ──────────────────────────────────────────────────────────────
function ItemCard({item,tags,rooms,storages,containers,canEdit,canAdmin,setModal,deleteItem,rawDeleteItem,returnItem,markOos,compact,T}){
  ...
  return(<div style={{background:oos?T.cardOos:T.cardBg,border:`0.5px solid ${T.border}`,borderRadius:compact?8:12,padding:compact?8:12,position:"relative",opacity:oos?.65:1}}>
    ...
    {(canEdit||canAdmin)&&(<div style={{display:"flex",gap:3,marginTop:7,flexWrap:"wrap"}}>
      ...
      {canEdit&&markOos&&(item.perishable
        ...
        :<ActBtn onClick={()=>setModal({type:"confirmDelete",title:"Discard Item?",message:`Are you sure you want to discard "${item.name}"? This will permanently delete it.`,onConfirm:()=>(rawDeleteItem||deleteItem)(item.id)})} T={T}>Discard</ActBtn>
      )}
