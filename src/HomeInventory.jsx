...
function ItemCard({item,tags,rooms,storages,containers,canEdit,canAdmin,setModal,deleteItem,returnItem,markOos,compact,T}){
  ...
  return(<div style={{background:oos?T.cardOos:T.cardBg,border:`0.5px solid ${T.border}`,borderRadius:compact?8:12,padding:compact?8:12,position:"relative",opacity:oos?.65:1}}>
    ...
    {(canEdit||canAdmin)&&(<div style={{display:"flex",gap:3,marginTop:7,flexWrap:"wrap"}}>
      ...
      {canEdit&&markOos&&<ActBtn onClick={()=>markOos(item.id)} T={T}>{oos?"✓ Back":"Out of Stock"}</ActBtn>}
...
function RoomsSection({rooms,storages,containers,items,addRoom,cascadeDeleteRoom,setConfirmDel,T}){
  ...
  const [name,setName]=useState("");
  const submit=()=>{if(!name.trim())return;addRoom({icon,name});setName("");setIcon("🏠");};
  const handleDel=(id)=>{
  ...
  return<PlacesSec title="🏠 Rooms" T={T}>
    <div style={{display:"flex",gap:8,marginBottom:14}}>
      <input value={icon} onChange={e=>setIcon(e.target.value)} placeholder="🏠" style={{width:52,...iS(T),textAlign:"center"}}/>
      <input value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submit()} placeholder="Room name (press Enter or click Add)" style={{flex:1,...iS(T)}}/>
