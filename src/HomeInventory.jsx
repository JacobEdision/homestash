...
export default function App() {
  ...
  return(
    <div style={{display:"flex",height:"100vh",overflow:"hidden",fontFamily:"system-ui,sans-serif",fontSize:14,background:T.bg,color:T.text}}>
      {/* Sidebar */}
      <Sidebar view={view} nav={nav} canAdmin={canAdmin} isSuperAdmin={isSuperAdmin} currentUser={currentUser} lendLogs={lendLogs} filterConsumable={filterConsumable} setFilterConsumable={setFilterConsumable} exportCSV={exportCSV} cycleUser={cycleUser} T={T} collapsed={!sidebarOpen}/>
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:16}}>
          ...
          {view==="users"&&<UsersView users={users} currentUserId={currentUserId} canAdmin={canAdmin} isSuperAdmin={isSuperAdmin} houses={houses} changeRole={changeRole} T={T}/>}
          ...
          {view==="superadmin"&&<SuperAdminView users={users} houses={houses} rooms={rooms} items={items} storages={storages} containers={containers} moveLogs={moveLogs} lendLogs={lendLogs} isSuperAdmin={isSuperAdmin} T={T}/>}
        </div>
      </div>
...
// ─── SIDEBAR ────────────────────────────────────────────────────────────────
function Sidebar({view,nav,canAdmin,isSuperAdmin,currentUser,lendLogs,filterConsumable,setFilterConsumable,exportCSV,cycleUser,T,collapsed}){
  ...
  if(collapsed){
    const icons=isSuperAdmin
      ...
      :[{v:"home",icon:"🏠"},{v:"allitems",icon:"📦"},{v:"lentout",icon:"🤝",badge:lentCount},{v:"auditlog",icon:"📋"},
    ...
    return(
      <div style={{width:52,background:T.sidebar,borderRight:`0.5px solid ${T.border}`,display:"flex",flexDirection:"column",flexShrink:0,alignItems:"center",paddingTop:8}}>
        ...
        <div style={{flex:1}}/>
        <div onClick={cycleUser} title={currentUser.name} style={{width:32,height:32,borderRadius:"50%",background:"#E6F1FB",color:"#0C447C",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:500,cursor:"pointer",marginBottom:12}}>{currentUser.name.slice(0,2).toUpperCase()}</div>
      </div>
    );
  }
        ...
        <div ref={ref} style={{position:"relative"}}>
          ...
          {showOos&&<div style={{position:"absolute",top:40,right:0,width:340,maxHeight:440,overflowY:"auto",background:T.cardBg,border:`0.5px solid ${T.border}`,borderRadius:12,boxShadow:"0 8px 32px rgba(0,0,0,.2)",zIndex:100}}>
            ...
            {!oosCount
              ...
              :oosItems.map(i=>{const room=rooms.find(r=>r.id===i.roomId);const storage=storages.find(s=>s.id===i.storageId);const t=i.tagIds.map(tid=>tags.find(x=>x.id===tid)).filter(Boolean);return(
...
function HomeView({T,roomGroups,visibleRooms,canAdmin,nav,items,tags,storages,containers,search,filteredItems,canEdit,setModal,deleteItem,returnItem,markOos}){
  if(search){const found=filteredItems(items);return<div><div style={{fontSize:16,fontWeight:500,marginBottom:14,color:T.text}}>Search Results <span style={{fontSize:13,fontWeight:400,color:T.textSec}}>({found.length} items)</span></div>{!found.length?<div style={{color:T.textSec,textAlign:"center",padding:32}}>No items match "{search}"</div>:<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>{found.map(i=><ItemCard key={i.id} item={i} tags={tags} rooms={visibleRooms} storages={storages} containers={containers} canEdit={canEdit} canAdmin={canAdmin} setModal={setModal} deleteItem={deleteItem} returnItem={returnItem} markOos={markOos} T={T}/>)}</div>}</div>;}
  ...
  return(<div style={{background:oos?T.cardOos:T.cardBg,border:`0.5px solid ${T.border}`,borderRadius:compact?8:12,padding:compact?8:12,position:"relative",opacity:oos?.65:1}}>
    ...
    {(canEdit||canAdmin)&&(<div style={{display:"flex",gap:3,marginTop:7,flexWrap:"wrap"}}>
      ...
      {canEdit&&markOos&&(item.perishable
        ...
        :<ActBtn onClick={()=>setModal({type:"confirmDelete",title:"Discard Item?",message:`Are you sure you want to discard "${item.name}"? This will permanently delete it.`,onConfirm:()=>deleteItem(item.id)})} T={T}>Discard</ActBtn>
      )}
